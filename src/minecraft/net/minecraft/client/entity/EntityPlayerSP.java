...
import complex.event.impl.EventRespawn;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public double getBaseSpeed() {
        if (!isPotionActive(Potion.getPotionById(1)))
            return isSprint();
        return isSprint() * 1.0D + 0.06D * (getActivePotionEffect(Potion.getPotionById(1)).getAmplifier() + 1);
    }

    public boolean isMoving() {
        return (this.movementInput.moveForward != 0.01F || this.movementInput.moveStrafe != 0.01F);
    }

    public void moveEntity(double x, double y, double z) {
        EventMove em = new EventMove(x, y, x);
        em.call();
        super.moveEntity(MoverType.PLAYER, em.getX(), em.getY(), em.getZ());
    }
    ...
    private void onUpdateWalkingPlayer() {
        EventUpdate em = new EventUpdate(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch, this.isSneaking(), this.onGround);
        EventUpdate em1 = new EventUpdate();
        em.call();
        if (em.isCancelled()) {
            return;
        }
        ...
        if (this.isCurrentViewEntity()) {
            ...
            double d3 = em.getYaw() - this.lastReportedYaw;
            double d4 = em.getPitch() - this.lastReportedPitch;
            ...
            if (this.isRiding()) {
                ...
                flag2 = false;
            } else if (flag2 && flag3 || em.shouldAlwaysSend()) {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(em.getX(), em.getY(), em.getZ(), em.getYaw(), em.getPitch(), em.isOnground()));
            } else if (flag2) {
                this.connection.sendPacket(new CPacketPlayer.Position(em.getX(), em.getY(), em.getZ(), em.isOnground()));
            } else if (flag3) {
                this.connection.sendPacket(new CPacketPlayer.Rotation(em.getYaw(), em.getPitch(), em.isOnground()));
            } else if (this.prevOnGround != this.onGround) {
                this.connection.sendPacket(new CPacketPlayer(em.isOnground()));
            }
    ...
    public void sendChatMessage(String message) {
        if (Complex.getCommandManager().execute(message) || message.startsWith("-")) {
            return;
        }
        if (Complex.getSettingsManager().getSettingByName("sendGarbled").getValBoolean()) {
            try {
                message = new String(message.getBytes("UTF-8"), "Shift-JIS");
    ...
    public void respawnPlayer() {
        EventRespawn respawn = new EventRespawn();
        respawn.call();
    ...
    public void onLivingUpdate() {
        ...
        if (this.isHandActive() && !this.isRiding()) {
            if (!Complex.getModuleManager().isEnabled("NoSlowdown")) {
                this.movementInput.moveStrafe *= 0.2F;
                this.movementInput.moveForward *= 0.2F;
                this.sprintToggleTimer = 0;
            } else {
                if (Complex.getSettingsManager().getSettingByName("NoSlow Mode").getValString().equalsIgnoreCase("AAC")) {
    ...
    public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
        EventMove em = new EventMove(p_70091_2_, p_70091_4_, p_70091_6_);
        em.call();
        double d0 = this.posX;
        double d1 = this.posZ;
        super.moveEntity(x, em.getX(), em.getY(), em.getZ());
